...
public class JobDetailImpl implements Cloneable, java.io.Serializable, JobDetail {
    ...
    */
    ...
    public boolean isConcurrentExectionDisallowed() {
        ...
        boolean diAllowed =  ClassUtils.isAnnotationPresent(jobClass, DisallowConcurrentExecution.class);
        if(!diAllowed){
            String key = SchedulerUtils.getJobKey(getName(), getGroup());
            QuartzJobDetail jobDetail = SchedulerUtils.jobMap.get(key);
            if(jobDetail!=null && jobDetail.getType()==1){
                return true;
            }
        }
        return diAllowed;
    ...
    @Override
    public Object clone() {
        org.quartz.impl.JobDetailImpl copy;
        try {
            copy = (org.quartz.impl.JobDetailImpl) super.clone();
            if (jobDataMap != null) {
